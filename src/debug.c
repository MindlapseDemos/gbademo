...
uint16_t vblperf_color[] = {
	/* ltgr white  grn    blue   cyan    yellow   orng    red   purple  d.green white ... */
...
void panic_impl(void *pc, const char *fmt, ...)
{
	int y;
	va_list ap;
	uint32_t *reg;
	...
	gba_setbgcolor(0, 31, 0, 0);
	gba_setbgcolor(0xff, 31, 31, 31);

	fill_16byte((void*)VRAM_LFB_FB0_ADDR, 0, 240 * 160 / 16);

	fill_16byte((unsigned char*)VRAM_LFB_FB0_ADDR + 240 * 3, 0xffffffff, 240 / 16);
	...
	fill_16byte((unsigned char*)VRAM_LFB_FB0_ADDR + 240 * (y + 4), 0xffffffff, 240 / 16);
	y += 8;
